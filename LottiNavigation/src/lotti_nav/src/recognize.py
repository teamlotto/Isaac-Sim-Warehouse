...
class Recognizer():
    ...
    def get_bbox_width(self, class_name: str,prb_thr: float = 0.9, width_thr: float = 196.) -> dict:
        ...
        if class_name in bbox_dict.keys():
            ...
            for idx, infos in enumerate(bbox_dict[class_name]):
                ...
                if prb < prb_thr or (xmax - xmin) < width_thr:
                    pass_err += 1
                    continue
                bbox_width_dict[idx-pass_err] = xmax - xmin

        return bbox_width_dict

    def get_bbox_center(self, class_name: str, prb_thr: float = 0.9, width_thr: float = 175.) -> dict:
        bbox_dict = self.get_bbox_info()
        center_dict = {}
        pass_err = 0
        if class_name in bbox_dict.keys():
